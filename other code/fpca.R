FPCAtest = function(y,tt,rr,H0="mean",B=500,p.adjust.methods="BH",FDR=0.05,miss=1,
           bwxcov=c(0,0),ngrid=51,ngrid1=30,
           kern="gauss",error=1,selection_k="FVE",FVE_threshold=0.9,verbose="on")
### INPUT ###
...
# id.sig: index for subjects that reject the null hypothesis
{
    ...
    r = permtest(res,y,tt,rr,miss,H0,B,FDR)
    p = r$p
    stat = r$stat
...
permtest = function(res,y,tt,rr,miss=1,H0="mean",B=500,delta=res$sigma)
### INPUT ###
# res: output from PCA.
...
#        the variance; default value is estimated measurement error variance (see 
#        output sigma in PCA); when setting to 0, the regular F statistics are used.
### OUTPUT ###
# p: vector of pvalues
# stat: vector of statistics
{
...
###########ÔÚÕâÀï
PCA = function(y,tt,rr,miss=1,bwxcov=c(0,0),ngrid=30,ngrid1=30,kern="gauss",error=1,
               selection_k="FVE",FVE_threshold=0.9,verbose="on")#ngrid=51
### INPUT ###
# y: n*m data matrix, with missing values denoted as NA.
# tt: length mm vector, unique time points.
# rr: number of repetitions at each unique time point.
...
getEigens = function(xcov,out1,out21,noeig,disp=FALSE)
{
  ...
  if(length(idx) > 0){
       if(disp)
          cat(paste(length(idx), "real eigenvalues are negative or zero and are remove!\n"))
       eigens = eigens[,d>0]
       d = d[d>0]
  }
